...
HOME <- "Y:/TKeggin/genesis/v0.9.9/config/"
setwd(HOME)
...
write_csv(params_table, "./1_parameter_exploration/config_parameters.csv")
...
config_dir <- paste0("./config/1_parameter_exploration/", 1:25, '.R')
...
write(c(run_head, run_body), file='1_parameter_exploration_1-25.bat')

#create bat file
run_head <- '@runAsMultiple,@Node_NODE15'
script_name <- 'run_GaSM.R'
r_version <- "/_shared/R3.6.1/r-with-tools.bat"
config_dir <- paste0("./config/1_parameter_exploration/", 26:50, '.R')
input_dir <- '-i ../input/1d_2000m_17c/'
output_dir <- '-o output/1d_2000m_17c/'
other_par <- '-s all'

run_body <- paste(r_version, script_name, config_dir, input_dir, output_dir, other_par)
write(c(run_head, run_body), file='1_parameter_exploration_26-50.bat')
