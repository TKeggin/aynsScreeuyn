...
continuity <- function(metric1,metric2){
  ...
  metric1_normalised <- (metric1-max(metric1_clean))/(max(metric1_clean)-min(metric1_clean))
  metric2_normalised <- (metric2-max(metric2_clean))/(max(metric2_clean)-min(metric2_clean))
