...
for(timestep in timesteps.seq){
  ...
  pa_dataframe             <- data.frame(matrix(0,nrow=length(coords[,1]), ncol=(length(species)+2))) # create a dataframe
  pa_dataframe[,1:2]       <- coords # add the coordinate information
  rownames(pa_dataframe)   <- rownames(coords) # set rownames as cell IDs
